...
const { checkCode, deleteCode } = require('../service/changeCode.service')
// 格式化时间的包
const moment = require('moment')
// 关闭提示
moment.suppressDeprecationWarnings = true;

const verifyForgotCode = async (ctx, next) => {
  // code使用的时间,输入的code，请求体里的数据
  const { email, code, codeTime } = ctx.request.body
  // 生成写入数据库的code
  ...
  if (res === '查询失败') {
    ...
    // 查到res的情况,证明请求的邮箱和code与数据库里的匹配了
     ...
     // 格式化输入框输入code发送请求时间
     const useCodeTime = moment(codeTime,moment.ISO_8601).format('YYYY-MM-DD HH:mm:ss')
     // 两个时间相减
